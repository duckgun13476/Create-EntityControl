...
public class Config {
    ...
    private static final ForgeConfigSpec.IntValue SQUEEZE_DESTROY_SPEED =
            BUILDER.comment("10x of destroy speed")
    ...
    private static final ForgeConfigSpec.ConfigValue<List<? extends String>> BLACKS_STRING =
            BUILDER.comment("A list of items can't be moved.")
    ...
    public static int squeeze_destroy_speed;
    public static String magicNumberIntroduction;
    public static Set<Item> items;
    public static Set<Item> blocks_unmoved;
    ...
    static void onLoad(final ModConfigEvent event) {
        ...
        squeeze_destroy_speed = SQUEEZE_DESTROY_SPEED.get();
        magicNumberIntroduction = MAGIC_NUMBER_INTRODUCTION.get();

        // convert the list of strings into a set of items
        items = ITEM_STRINGS.get().stream().map(itemName -> ForgeRegistries.ITEMS.getValue(new ResourceLocation(itemName))).collect(Collectors.toSet());
        blocks_unmoved = BLACKS_STRING.get().stream().map(itemName -> ForgeRegistries.ITEMS.getValue(new ResourceLocation(itemName))).collect(Collectors.toSet());
    }
}
